...
def fn_ConvertFrameRGB2GRAY(DictOfVideoCaptureDataObjects):
    ...
    ## TEST PRINT OUTPUT
    print(f"\nBEGIN FUNCTION #00 - CONVERT FRAME FROM RGB TO GRAYSCALE")
    ...
    for k, VideoCaptureDataObject in DictOfVideoCaptureDataObjects.items():
        ...
        for EachFrame in ListForVideoGIFFramesRGB:
            ...
            EachFrameGray = cv2.cvtColor(EachFrame, cv2.COLOR_RGB2GRAY)
    ...
    ## TEST PRINT OUTPUT
    print(f"\nEND FUNCTION #00 - CONVERT FRAME FROM RGB TO GRAYSCALE")
